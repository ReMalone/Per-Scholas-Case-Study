...
public class LoginServlet extends HttpServlet {
		...
		try {
			...
			if(user != null)
			{
				...
				if(policyList.isEmpty())
				{
					...
					req.setAttribute(HomeInsuranceConstants.MESSAGE, "User has no policies");
				}
				else
				{
	...
	private void loginNewUser(final HttpServletRequest req,
		...
		try {
			final LoginBO loginBO = new LoginBO();
			User duplicate = loginBO.getUser(userName);
			System.out.println(loginBO.getUser(userName));
			...
			if(duplicate == null)
			{
				...
				req.setAttribute(HomeInsuranceConstants.MESSAGE, "Username is already taken.");
				final RequestDispatcher dispatcher = req.getRequestDispatcher("newUser.jsp");
				dispatcher.forward(req, res);
				} 
			} catch (Exception e) {
				logger.error("Exception occurred while executing method LoginServlet.doGet :: " + e);
	...
	private void loginExistingUser(final HttpServletRequest req,
		...
		try {
			...
			if(user != null)
			{
				if(user.getUserRole() != null && user.getUserRole().equals("Admin") && user.getPassword().equals(password))
				{ 
						session.setAttribute("Role", "Admin");
						page = HomeInsuranceConstants.SEARCH_USER;
				}
				else
				{ System.out.println("The login user went through LoginServlet line 84");
					session.setAttribute(HomeInsuranceConstants.USER_NAME, userName);
					if(!user.getPassword().equals(password))
					{
						req.setAttribute(HomeInsuranceConstants.MESSAGE, "Invalid User Name or Password");
						page = "/welcome.jsp";
						final RequestDispatcher dispatcher = req.getRequestDispatcher(page);
						dispatcher.forward(req, res);	
					...
					//Get Policy
					final PolicyBO policyBO = new PolicyBO();
					System.out.println("Code got to here");
